...
protected:
  void SetUp() override {
    ...
    xml_str =
        psu::utilities::readFile(g_program_path + "/sep_xml/ResponseSet.xml");
    list_xml_str = psu::utilities::readFile(g_program_path +
                                            "/sep_xml/ResponseSetList.xml");
  }
...
TEST_F(TestResponseSetXML, IsAdapterTranslationAccurate) {
  ...
  EXPECT_EQ(rsps.description.value(), "description1");
  EXPECT_EQ(rsps.version.value(), 0);
  EXPECT_EQ(rsps.response_list_link.value().all, 0);
  EXPECT_EQ(rsps.response_list_link.value().href, "http://uri1");
}
