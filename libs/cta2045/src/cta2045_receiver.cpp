...
CTA2045Receiver::CTA2045Receiver(CombinedHttpsClient *c)
    : epri_ucm_(&response_, c, &timer_), serial_port_("/dev/ttyAMA0"),
      shutdown_(false) {
  ...
  if (!serial_port_.open()) {
    ...
    std::string msg_out =
        xml_writer_.WriteMsg("DCM", "DER", "CTA2045_Message", "CTA2045_Startup",
                             "Startup... Initiating CTA2045 Communications");
    c->Post("DTM", msg_out);
...
std::string CTA2045Receiver::GetNameplate() {
  ...
  log = "CTA2045: " + response_; // for command
  response_.erase();
  std::cout << " cta2045receiver::getNamePlate GetNameplate() log: " << log
            << std::endl;

  return log + ", elapsed: " + std::to_string(elapsed);
  // return xml_writer_.ReturnCustomCommand("DCM", "DER", response, "na", "na",
  // "response");
}
...
std::string CTA2045Receiver::GridEmergencyEvent() {
  ...
  log = "CTA2045: " + response_; // for command
  response_.erase();
  std::cout << " cta2045receiver GridEmergencyEvent() log: " << log
            << std::endl;
