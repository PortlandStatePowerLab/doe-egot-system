  ...
  struct send_lambda {
    ...
    void operator()(http::message<isRequest, Body, Fields> &&msg) const {
      ...
      http::async_write(self_.stream_, *sp,
                        std::bind(&Session::loop, self_.shared_from_this(),
...
public:
  ...
  explicit Session(boost::asio::ip::tcp::socket &&socket,
    ...
    stream_.set_verify_callback(boost::bind<bool>(
        &Session::IDVerifyCallback, this, false, boost::placeholders::_2));
  }
  ...
  void run() {
    ...
    net::dispatch(stream_.get_executor(),
                  beast::bind_front_handler(&Session::loop, shared_from_this(),
                                            beast::error_code{}, 0, false));
  }

  bool IDVerifyCallback(bool preverified,
    ...
    case X509_V_ERR_UNABLE_TO_DECODE_ISSUER_PUBLIC_KEY:
      std::cout << "the public key in the certificate SubjectPublicKeyInfo "
                   "could not be read"
                << std::endl;
      break;
...
#include <boost/asio/yield.hpp>
  ...
  void loop(beast::error_code ec, std::size_t bytes_transferred, bool close) {
    ...
    reenter(*this) {
      ...
      yield stream_.async_handshake(ssl::stream_base::server,
      ...
      yield stream_.async_shutdown(std::bind(&Session::loop, shared_from_this(),
                                             std::placeholders::_1, 0, false));
      if (ec)
