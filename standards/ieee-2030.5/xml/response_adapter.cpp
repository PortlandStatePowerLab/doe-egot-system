...
namespace xml
{
    ...
    {
        boost::property_tree::ptree* pt;
        pt->put("ResponseList.<xmlattr>.results", rsp_list.size());

        for (const auto& rsp : rsp_list)
        {
            boost::property_tree::ptree pt2;
            TreeMap(rsp, &pt2);
            pt->add_child("ResponseList.Response", pt2);
        }
    ...
    {
        boost::property_tree::ptree pt = xml::util::Treeify(xml_str);
        BOOST_FOREACH (boost::property_tree::ptree::value_type &subtree, pt.get_child("ResponseList.Response"))
        {
            sep::Response* rsp;
