...
int main(int, char *[]) {
  ...
  flecs::entity sun =
      ecs.entity("Sun").add<Position, World>().set<Position, Local>({1, 1});
  ...
  ecs.entity("Venus").child_of(sun).add<Position, World>().set<Position, Local>(
      {2, 2});
  ...
  // Do the transform
  q.iter([](flecs::iter &it, const Position *p, const Position *p_parent,
            Position *p_out) {
    for (auto i : it) {
      auto parent = it.entity(i).parent();
      std::cout << it.entity(i).path() << " , " << parent.path() << std::endl;
      p_out[i].x = p[i].x;
      p_out[i].y = p[i].y;
      if (p_parent) {
